...
使用 GitHub 官方 `github/gh-stack` 扩展。`origin` 指向个人 fork，远端操作明确使用它：

```sh
GH_REPO=happy-proto/tinycast gh stack view --json
```

支持 `--remote` 的写命令一律指定 `--remote origin`。若裸 PR 编号被解析到上游，使用
`GH_REPO=happy-proto/tinycast` 和完整的 fork PR URL。

`gh-stack` 的部分命令从名为 `origin` 的 remote 写入本地 stack 仓库身份，忽略 `GH_REPO`。
导入或写入后检查 `.git/gh-stack` 的 `repository`、目标 stack 的 REST URL 和 PR URL，
不能只信命令成功输出。若目标被解析成上游，停止继续写入并检查 remote 配置。
...
```sh
GH_REPO=happy-proto/tinycast gh stack rebase --upstack --remote origin
GH_REPO=happy-proto/tinycast gh stack push --remote origin
```
...
- 创建或更新 PR 并重新关联 stack 时使用 `gh stack submit --remote origin`。
- 不擅自改变 draft 状态。
...
官方 unstack 操作；若扩展仍路由错误，可对已确认的 fork URL 调用同一个官方
