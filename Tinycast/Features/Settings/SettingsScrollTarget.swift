...
extension SettingsSectionHeader where Label == Text {
    ...
    init(_ anchor: SettingsAnchor) {
        self.init(anchor: anchor) { Text(SettingsLocalization.string(anchor.title)) }
    }
}
...
struct SettingsRowTitle: View {
    ...
    var body: some View {
        Text(SettingsLocalization.string(title)).modifier(SearchPill(target: .row(anchor, title)))
    }
}
